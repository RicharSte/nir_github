...
def get_repositories():
    ...
    while page <= page_amount:
        ...
        if response.status_code == 200:
            ...
            repositories.extend([{'name': repo['name'], 'owner': repo['owner']['login']} for repo in repos])
            page += 1
        else:
    ...
    print(f'Список репозиториев сохранен в файле "repositories.json". Всего репозиториев: {len(repositories)}')
