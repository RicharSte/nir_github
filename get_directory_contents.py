...
FileData = namedtuple(
    'FileData', ['owner', 'repo_name', 'file_name', 'file_content']
)


def get_directory_contents(
    owner: str, repo_name: str, directory_path='', results=None
):
    if results is None:
        results = []
            ...
            if content_item['type'] == 'file':
                ...
                if file_response.status_code == 200:
                    ...
                    if file_name.endswith('.zip'):
                        try:
                            extract_zip(
                                file_content, owner, repo_name, results
                            )
                    ...
                    elif file_name.endswith('.tar.gz') or file_name.endswith(
                        '.tgz'
                    ):
                        try:
                            extract_tar(
                        ...
                        try:
                            ...
                            print(
                                f'Ошибка при декодировании файла {file_name}'
                            )
...
def extract_tar(file_content, owner, repo_name, results):
    with tarfile.open(
        fileobj=io.BytesIO(file_content), mode='r:gz'
    ) as tar_ref:
...
def extract_7z(file_content, owner, repo_name, results):
    try:
        ...
        print(f'Файл использует неподдерживаемый метод сжатия')
    except Exception as e:
